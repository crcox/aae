...
% endif
  ...
  loadExamples [file join "ex" "train.ex"] -exmode ORDERED
  ...
  set numTrainingExamples [getObj trainingSet.numExamples]
  if {[string equal $MaxIter LengthOfSequence]} {
    set MaxIter $numTrainingExamples
  }
  ...
  set errHistory [list]
  set i 0
  while {$PError <%text>></%text> $ErrorCriterion <%text>&&</%text> $i <%text><</%text> $MaxIter } {
    train -a steepest
