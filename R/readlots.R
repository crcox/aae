...
A <- c("Blocked_accent","Interleaved_accent")
B <- c("strong") # weak not run
C <- c("AAE","SAE")
D <- 0:9
...
for (r in 1:nrow(M)) {
  m <- M[r,]
  fpath <- file.path(root,M$condition[r],M$dialect[r],M$accent[r],M$iter[r],"phase01_MFHC.csv")
